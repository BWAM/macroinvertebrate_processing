...
sites_survey_23<-readxl::read_excel("L:/DOW/BWAM Share/data/data_warehouse_staging/2024_V3/raw_data/stream/siteIdCrosswalkTable2023_v3_2025_01_30.xlsx")
sites_survey_24<-readxl::read_excel("L:/DOW/BWAM Share/data/data_warehouse_staging/2024_V3/raw_data/stream/siteIdCrosswalkTable2024_v4_2025_01_27.xlsx")


sites_23<-sites_survey_23 %>% 
  select(finalSiteID,revisedOriginalSiteID, eventSMASSampleDate,eventSMASArrivetime,eventSMASId) %>% 
  mutate(EVENT_ID = eventSMASId,
         DATE = eventSMASSampleDate) %>% 
  mutate(DATE = as.Date(DATE, "%m/%d/%y")) %>% 
  distinct() %>% 
  select(c(finalSiteID,revisedOriginalSiteID,EVENT_ID,DATE))

sites_24<-sites_survey_24 %>% 
  select(finalSiteID,revisedOriginalSiteID, eventSMASSampleDate,eventSMASArrivetime) %>% 
  mutate(DATE = eventSMASSampleDate) %>% 
  mutate(DATE = as.Date(DATE, "%m/%d/%Y")) %>% 
  mutate(EVENT_ID = paste(finalSiteID,"_",
                          format(DATE,'%Y%m%d'),
                          "T",
                          stringr::str_replace(eventSMASArrivetime,":",""),
                          sep = "")) |> 
  distinct() %>% 
  select(c(finalSiteID,revisedOriginalSiteID,EVENT_ID,DATE))
...
sites_23_24<-rbind(sites_23,
                   sites_24) %>% 
  rename(site_id = revisedOriginalSiteID,
         final_id = finalSiteID)# number of rows match here for the rbind
...
final_metrics_4<-final_metrics_4 %>% 
  ...
  mutate(EVENT_ID = case_when(
    site_id %in% "11-ARCH-1.9"~"11-ARCH-1.9_20230918T0000",
    ...
    site_id %in% "12-SIMB-0.5" ~ "10-SMIB-0.5_20230727T1230",
    TRUE ~ EVENT_ID
  ),
  final_id = case_when(
    ...
    site_id %in% "12-SIMB-0.5" ~ "10-SMIB-0.5",
    TRUE ~ final_id))
...
imported_metrics<-import_s_macro_metrics(data_dir) |> 
  select(-site_id) |> 
  rename(site_id = final_id) # we want the corrected site_ids
...
prepped_sample_table<-prepped_sample_table %>% 
  mutate(site_id = case_when(event_id %in% "11-ARCH-1.9_20230918T0000" ~ "11-ARCH-1.9",
...
prepped_metrics3<-prepped_metrics2 %>% 
  mutate(#CASRN = "not_applicable", # aeh: don't need these parameter columns, units already exists
