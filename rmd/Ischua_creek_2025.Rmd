...
params:
  file_name: "Copy of IschuaCreek.csv"
  user: kareynol
  file_type: "waa"
---
...
raw<-read.csv(here::here(paste("data",params$file_name,sep = "/")))
...
#saveRDS(bap.prepped,here::here("outputs/bap_prepped.RDS"))
#looking at zach's

zach<-readRDS(here::here("outputs/bap_prep_zs.RDS"))
...
#bap.final_zs<-apply_bap(zach)

bap.finalzach<-BAP::bap_riffle(zach)
...
setdiff(sites.event,sites.metrics) #this is the low gradient 02-ISCH-15.9

write.csv(bap.final,here::here("outputs/bap_ischua_check_2025.csv"))
```
Prep for WQMA
...
final_event<-samp_event

final_metrics<-metrics.subset
...
#our new coc will have time and event ids already populated, for this i had to create them, and check whats in the db already

final_metrics_3<-final_metrics_2 |> 
  mutate(EVENT_ID = case_match(site_id,
                              "02-ISCH-0.9" ~ "02-ISCH-0.9_20250828_T1245",
                              "02-ISCH-10.9" ~ "02-ISCH-10.9_20250828_T1730",
                              "02-ISCH-13.8" ~ "02-ISCH-13.8_20250828_T0845",
                              "02-ISCH-15.1" ~ "02-ISCH-15.1_20250828_T1200",
                              "02-ISCH-17.2" ~ "02-ISCH-17.2_20250828_T1015",
                              "02-ISCH-19.0" ~ "02-ISCH-19.0_20250828_T1200",
                              "02-ISCH-6.1"  ~ "02-ISCH-6.1_20250828_T1330",
                              "02-ISCH-8.7"  ~ "02-ISCH-8.7_20250828_T0945",
                              "02-OLEA-3.9"  ~ "02-OLEA-3.9_20250828_T1200",
                              .default = "error"))
...
sample_final_2<-sample_final_2 %>% 
  distinct()
#create check
final_metrics_4<-final_metrics_3 %>% 
  distinct()

if (nrow(sample_final_2) != nrow(final_metrics_4)) {stop("Number of rows don't match, check the matching")}

#save to file
 write_csv(sample_final_2,"L:/DOW/BWAM Share/data/data_warehouse_staging/2025/bap/bap_sample_ischua_2025.csv")
  write_csv(final_metrics_4,"L:/DOW/BWAM Share/data/data_warehouse_staging/2025/bap/bap_metrics_ischua_2025.csv")
...
data_dir<-"L:/DOW/BWAM Share/data/data_warehouse_staging/2025/bap/"
...
prep_s_macro_metrics <- function(x, s_event, s_macro_sample) {
  S_MACRO_METRICS_final <- x |> 
    ...
    distinct() |> 
    tidyr::pivot_longer(
      cols = c(score_bioassessment_profile:score_nutrient_biotic_index_phosphorus,total_individual_count),#this should be different bc you have to change it to fit your metrics - we shoudl look at something thats not as spelled out?
...
imported_metrics<-import_s_macro_metrics(data_dir) #some of the metrics are different but thats ok, we dont have them all in this file since its just kick and low gradient
...
names(prepped_metrics3) <- base::toupper(names(prepped_metrics3))
...
#update the result_qualifier
prepped_metrics4<-prepped_metrics3 |> 
  mutate(RESULT_QUALIFIER = case_when(RESULT_QUALIFIER %in% "" ~as.character(0),
                                      TRUE ~ RESULT_QUALIFIER),
         RESULT_CATEGORY = case_when(RESULT_CATEGORY %in% "" ~ "not_applicable",
                                     TRUE ~ RESULT_CATEGORY))
...
#warn about result_category and result_qualifier
...
event_ids<-final_metrics_4 |> 
  select(MSSIH_EVENT_SMAS_HISTORY_ID=site_id,EVENT_ID) |> 
  distinct()
#manually add in the missing one
missing_site<-"02-ISCH-15.9"
event_missing <- "02-ISCH-15.9_20250828_T1330"
# Combining vectors into a dataframe
df1 <- data.frame(MSSIH_EVENT_SMAS_HISTORY_ID = missing_site , 
                  EVENT_ID = event_missing)

event_ids2<-rbind(event_ids,df1)


raw_merge<-merge(raw,event_ids2,
                 by = "MSSIH_EVENT_SMAS_HISTORY_ID")


taxa_raw <- raw_merge |> #---------------- Renaming the preexisting columns that are given in the WAA report
...
#make sure there are not any missing sites
setdiff(unique(raw$MSSIH_EVENT_SMAS_HISTORY_ID),unique(taxa_final4$SITE_CODE))
