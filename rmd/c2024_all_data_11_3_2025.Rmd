---
title: "macroinvertebrate_processing_2024_remaining_data"
author: 'NYSDEC SMAS : Keleigh Reynolds'
date: "11/3/2025"
output: html_document
params:
  file_name: "L:/DOW/BWAM Share/data/lab_data/WAA/2025/2024MacroDataNYSDEC_kar_adjustments_date_time.csv"
  user: kareynol
  file_type: "waa"
---
...
RMN<-raw |> 
  filter(MSSIH_BIOSAMPLE_COLLECT_METHOD %in% "Kick: RMN 300 Count")

#fix the methods
raw<-raw |> 
  mutate(MSSIH_REPLICATE = case_when(
    MSSIH_BIOSAMPLE_COLLECT_METHOD %in% "Kick: RMN 300 Count" ~ 2, #adding a different rep # to distinguish between the 2 methods for RMN
    TRUE ~ MSSIH_REPLICATE),
    MSSIH_BIOSAMPLE_COLLECT_METHOD =
           case_when(
             MSSIH_BIOSAMPLE_COLLECT_METHOD %in% "Kick: Headwater Stream" ~ "Kick: Standard",
             MSSIH_BIOSAMPLE_COLLECT_METHOD %in% "Kick: RMN 300 Count" ~ "Kick: Standard",
             MSSIH_BIOSAMPLE_COLLECT_METHOD %in% "Lake: Littoral" ~ "Littoral",
...
raw<-raw %>%
  ...
  mutate(MSDH_GENSPECIES = case_when(
    MSDH_GENSPECIES %in% "neoleptophlebia_sp."~"leptophlebia_sp.",
...
#bring in the taxonomy so we can correct that
taxa_result_dir <- file.path(
  "L:",
  "DOW",
  "BWAM Share",
  "data",
...
#collect taxa and fix
taxa_df <- open_dataset(taxa_result_dir) |> 
  collect()
#need to correct neoleptophlebia_sp. to neoleptophlebia (in the taxon table, corrected above)
master_taxa<-BAP::master

setdiff(unique(raw$MSSIH_EVENT_SMAS_HISTORY_ID),unique(prepped_df$MSSIH_EVENT_SMAS_HISTORY_ID))
#all good none are missing, same # of rows in raw and prepped files

#have to figure out the missing species guy
...
setdiff(sites.event,sites.metrics) #These are the low gradient guys and the lake ones , good to keep in there
#  [1] "0301UWB0001-0.0" "04-BLAK_T16-0.8" "04-RECK_T1-0.6"  "07-REDC_T03-4.3" "0701UWB5470-0.0"
#  [6] "09-BLCH-0.2"     "09-GRAS_S-40.3"  "09-SREG-0.2"     "09-TURN-1.5"     "0902BLA0256-0.0"
# [11] "1004LCA0270-0.0" "11-LHUD_T95-0.1" "13-GILD-0.1"     "13-MOHL_T02-0.1" "13-MUDK-6.1"    
# [16] "13-NTRIBGL-0.06" "1401UWB0260-0.0"
```
Prep for WQMA
...
#it didnt this is because of the low gradient and some lakes as noted above

#create the event id, need time - KAR added these in the raw file - what a shit show that was, so i'm joining them here

event_id<-raw |>
  select(event_id,site_id =MSSIH_EVENT_SMAS_HISTORY_ID) |>
  distinct()

final_metrics_3<-left_join(final_metrics_2,event_id,
                       by = "site_id")


#write to folder 
write.csv(final_metrics_3,"L:/DOW/BWAM Share/data/lab_data/BAP_script_results/2025/2024_110425_kar_metrics.csv") #done on 10/14/25 KAR
```



```{r create-new-sample-final}
#create new sample file as well - this is missing hte low gradient and lakes ones - let's fix

sample_final<-samp_event %>% 
  select(c(starts_with("MSSIH_")))

sample_final_2<-sample_final %>% 
  distinct()
#create check
final_metrics_3<-final_metrics_2 %>% 
  distinct()

if (nrow(sample_final_2) != nrow(final_metrics_2)) {stop("Number of rows don't match, check the matching")} #that's fine its the ones tha tdidnt go through BAP

event_id <- raw |> 
  select(MSSIH_EVENT_SMAS_HISTORY_ID,
         event_id) |> 
  distinct() #221 total samples, so this is ok - that accounts for replicates

#merge event_id
sample_final_3<-left_join(sample_final_2,event_id,
                      by = "MSSIH_EVENT_SMAS_HISTORY_ID")

#write to folder 
write.csv(sample_final_3,"L:/DOW/BWAM Share/data/lab_data/BAP_script_results/2025/all2024_sample_event_kar.csv") #done on 10/14/25 KAR
```
...
#need to check these
#   [1] "04-PBLO-3.3"     "04-RAGA-29.4"    "04-RAGA-33.0"    "05-BEEC-0.6"     "05-GOOD_T01-0.1" "06-SHDW-0.8"     "07-FALC-4.4"     "09-DBPD_T1-0.1" 
#  [9] "09-MPTR-0.1"     "09-MULL-10.2"    "09-SDSH-0.8"     "09-WMULL-2.0"    "10-CLEB-1.6"     "11-ADCR-4.5"     "11-BIRB-2.3"     "11-BRYM-0.1"    
# [17] "11-SACN_E-9.8"   "11-STWT-0.1"     "13-ETRIBGL-0.15" "13-FISH-21.21"   "13-GRAC-3.9"     "13-MURS-3.0"     "13-WICBR-0.28"   "13-WTRIBGL-0.06"
# [25] "14-CHAM-0.3"     "14-MELB-1.0"     "14-MONG_M-7.5"   "14-WLAK-0.6" 

```

```{r clean up missing sites}
...
base_dir <- file.path(
  ...
  "raw_data",
  "stream",
  "RE_ 23-24 stream survey links"
)

stream_sei_obt <- readxl::read_xlsx(
  file.path(base_dir,
            "2024_SEI_UserPercep_Survey.xlsx")
) |>
  janitor::clean_names("screaming_snake")


sei_short<-stream_sei_obt |> 
  filter(REVISED_ORIGINAL_SITE_ID %in% missing_sites)

missing_final<-stream_sei_obt |> 
  filter(!REVISED_ORIGINAL_SITE_ID %in% missing_sites)

x<-setdiff(missing_sites, unique(sei_short$REVISED_ORIGINAL_SITE_ID))

missing_sites_info<-raw |> 
  filter(MSSIH_EVENT_SMAS_HISTORY_ID %in% x ) |> 
  select(MSSIH_EVENT_SMAS_HISTORY_ID,MSSIH_EVENT_SMAS_SAMPLE_DATE,MSSIH_REPLICATE,
         Client_Project_Name,event_id) |> 
  distinct()
...
site_corrections <- function(col) {
  stringr::str_replace_all(
    ...
    c("12-SAUQ-2.2" = "12-SAUQ-1.7",
      ...
      "04-PBLO-3.3" = "04-PLBO-3.3",
      "13-FISH-21.21" = "13-FISH-21.2",
      "13-WTRIBGL-0.06" = "13-WTRIBGL-0.1",
      "13-ETRIBGL-0.15" = "13-ETRIBGL-0.1",
      "13-WICBR-0.28" = "13-WICBR-0.3"
...
macro_event_corrections <- function(df) {
  mutate(
    ...
    event_id = case_match(
      ...
      "10-JACK-0.2_20230726T1030" ~ "10-JKBR-0.2_20230726T1030",
      "04-PLBO-3.3_20240711T0845" ~ "04-PBLO-3.3_20240711T0845",
      "13-ETRIBGL-0.15_20240717T1200" ~ "13-ETRIBGL-0.1_20240717T1200",
      "13-FISH-21.21_20240716T1600" ~  "13-FISH-21.2_20240716T1600", 
     "13-WICBR-0.28_20240717T1200" ~ "13-WICBR-0.3_20240717T1200",  
      "13-WTRIBGL-0.06_20240717T1600" ~ "13-WTRIBGL-0.1_20240717T1600",# fixing the site that doesn't match - same name as another waterbody 
      .default = event_id
    )
  ) 
}
...
setdiff(prepped_sample_table$event_id, imported_sample$event_id) #checking ot make sure there were no transcript errors - fixed these, they are in the list of the fixes
  ...
  standard_read_csv(
    file = file.path(data_dir,
           ...
           col_types = cols(
             ...
             #MMDH_NCO_RICH = col_double(), #these are not int he file for this one
...
prepped_sample_metric_match <-prepped_sample_table |> 
  filter(site_id %in% imported_metrics$site_id) #checking these

prepped_metrics<-prep_s_macro_metrics(imported_metrics, samp_event, prepped_sample_metric_match)
...
#these are good -its the lakes and the low gradients
...
event_ids_tc<-prepped_metrics2 %>%  #check this for littoral
...
prepped_sample_table2<-prepped_sample_table |> 
  mutate(sample_method = case_when(
    ...
    collection_method %in% "Ponar" ~ "lake_ponar",
    collection_method %in% "Lake: Littoral" ~ "lake_littoral",
    collection_method %in% "Littoral" ~ "lake_littoral",
...
prepped_metrics3<-prepped_metrics2 %>% 
  mutate(CASRN = "not_applicable",
         ...
         LAB_VALIDATION_LEVEL = "not_applicable",
...
rename(SAMPLE_LOCATION = SAMPLING_LOCATION,
  ...
  mutate(
    ...
    SAMPLE_LAB = "Watershed Assessment Associates (WAA)",
    SAMPLE_COMMENT = "none", 
    SAMPLE_TYPE = "macroinvertebrate_metrics")|> 
...
prepped_metrics3<-prepped_metrics3 |> 
  mutate(SAMPLE_METHOD = case_when(
    (SITE_ID %in% RMN$MSSIH_EVENT_SMAS_HISTORY_ID & REPLICATE == 2) ~ "rmn_kick",
    TRUE~SAMPLE_METHOD))
...
```{r validation}
# Setup -------------------------------------------------------------------
# Install the BWAM_ETL package to gain access to functions for table splitting and validation.
# install.packages("pak")
 pak::pak("BWAM/BWAM_ETL")
library("BWAM.ETL")
library(purrr)

# Read in or Prep your OBT ------------------------------------------------
# Example OBT to be replaced with your own OBT data frame.
obt_df <- prepped_metrics3
obt_df$SAMPLE_CREW<-""

# Split the OBT into the target tables ------------------------------------
# Vector of target tables
table_vec <- c("RESULT", "SAMPLE")

prep_list <- table_vec |>
  purrr::set_names() |>
  # prep_wqma_table
  purrr::map( ~ prep_wqma_table(
    df = obt_df,
    target_table_name = .x,
    add_uid_md5 = FALSE
  ))

# Example of how to extract a list element.
t1 <- prep_list$RESULT

# Run the validation for each target table --------------------------------
val_list <- table_vec |>
  purrr::set_names() |>
  purrr::map(~ validate(
    df = prep_list[[.x]],
...
# Example of how to check validation.
# In this example, you can see (89-91) the columns SUBSAMPLE_AMOUNT, ANALYSIS_DATETIME, and PREP_DATETIME
# are not the expected column type (e.g., numeric, posix, etc.)
val_list$RESULT
val_list$SAMPLE

# Get just the failed validation checks
fail_list <- table_vec |>
  purrr::set_names() |>
  purrr::map( ~ pointblank::get_agent_report(agent = val_list[[.x]],
                                             keep = "fail_states"))
# This will print out n amount of reports, where n = length(table_vec)
# You can use the arrows at the top left of the RStudio Viewer pane to look at all of the reports.
# If the report is empty, then there were no issues.
fail_list
...
taxa_final4<-taxa_final4 |> 
    mutate(SAMPLE_METHOD = case_when(
    (SITE_CODE %in% RMN$MSSIH_EVENT_SMAS_HISTORY_ID & REPLICATE == 2) ~ "rmn_kick",
    TRUE~SAMPLE_METHOD))
